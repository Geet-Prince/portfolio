...
import { Star, Search, GitFork, BookOpen, Code2, Folder, Calendar, Shield } from 'lucide-react';
              ...
              <div className="w-full relative aspect-[16/10] overflow-hidden border-b border-(--border-subtle) shrink-0 bg-(--surface)">
                ...
                {project.featured && (
                  <div className="absolute top-5 right-5 z-20 flex items-center gap-1.5 px-3 py-1.5 bg-black/80 backdrop-blur-md rounded-full text-white text-xs font-bold tracking-wide uppercase border border-white/10 shadow-lg">
                    <Star size={14} fill="currentColor" className="text-amber-400" aria-hidden="true" />
                    <span>Featured</span>
                  </div>
                )}
              </div>
        ...
        <div className="space-y-6 mb-12">
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between bg-(--card)/50 p-4 rounded-2xl border border-(--border-subtle) backdrop-blur-md">
            ...
            <div className="flex items-center gap-3 w-full lg:w-auto">
              <label htmlFor="repo-sort" className="sr-only">Sort repositories</label>
              <span className="text-sm font-bold text-(--muted) whitespace-nowrap" aria-hidden="true">Sort by:</span>
              <select
                id="repo-sort"
                value={sort}
          ...
          {/* Filters */}
          <div className="flex flex-wrap gap-2 pt-2" role="group" aria-label="Filter repositories by category">
            {categories.map(cat => (
              <button
                key={cat}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            <AnimatePresence mode="popLayout">
              {(showAllRepos ? filteredRepos : filteredRepos.slice(0, 6)).map((repo) => (
                <m.div
                  layout
                  ...
                  <div className="flex items-start justify-between mb-4 gap-4">
                    <a href={repo.html_url} target="_blank" rel="noreferrer" className="flex items-center gap-3 group/title flex-1 min-w-0">
                      <div className="p-2.5 bg-(--surface) rounded-xl group-hover/title:bg-(--accent)/10 transition-colors shrink-0">
                        <BookOpen size={20} aria-hidden="true" className="text-(--fg) group-hover/title:text-(--accent) transition-colors" />
                      </div>
                    ...
                    <div className="flex items-center gap-1.5 px-2.5 py-1 bg-(--surface) border border-(--border-subtle) rounded-lg shrink-0">
                      <Shield size={12} aria-hidden="true" className="text-(--muted)" />
                    ...
                    <div className="flex items-center gap-4">
                      {repo.language && (
                        <div className="flex items-center gap-1.5">
                          <span className="w-2.5 h-2.5 rounded-full bg-(--accent)" aria-hidden="true"></span>
